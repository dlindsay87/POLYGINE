...
class Player : public Thing
{
	...
	float _yaw = 0.0f;
      ...
      public:
	Player()
	    : Thing(0.0f, 1.0f, glm::vec3(0.0f), glm::vec3(0.0f),
		    glm::vec3(0.0f))
	{
	}
	...
	void handleEvents(CORE::Input *ip) override
	{
		...
		if (ip->isKeyDown(SDL_SCANCODE_UP) ||
		    ip->isKeyDown(SDL_SCANCODE_W)) {
			tweenRotation(false);
			_position += _inc * _face;
		}
		if (ip->isKeyDown(SDL_SCANCODE_DOWN) ||
		    ip->isKeyDown(SDL_SCANCODE_S)) {
			tweenRotation(true);
			_position -= _inc * _face;
		}
		if (ip->isKeyDown(SDL_SCANCODE_RIGHT) ||
		    ip->isKeyDown(SDL_SCANCODE_D)) {
			tweenRotation(
			    std::abs(_yaw * M_RAD2DEG - _orientation.z) > M_90);
			_position += _inc * _shoulder;
		}
		if (ip->isKeyDown(SDL_SCANCODE_LEFT) ||
		    ip->isKeyDown(SDL_SCANCODE_A)) {
			tweenRotation(
			    std::abs(_yaw * M_RAD2DEG - _orientation.z) > M_90);
			_position -= _inc * _shoulder;
		}
	...
	void tweenRotation(bool flip, float inc = 6.0f)
	{
		...
		if (std::abs(direction) <= inc)
			_orientation.z = goal;
		else
			_orientation.z += std::copysign(inc, direction);
	}
};
} // namespace ENT