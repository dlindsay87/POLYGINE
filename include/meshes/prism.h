...
class Prism : public Mesh
{
	...
	void _generatePrism()
	{
		...
		// The four vertices v0, v1, v2, v3 should be specified in order
		// (clockwise or counter-clockwise)
		auto addFace = [this](glm::vec3 v0, glm::vec3 v1, glm::vec3 v2,
				      glm::vec3 v3) {
			GLuint startIndex = _vertices.size();

			_vertices.insert(_vertices.end(),
					 {v0, v1, v2, v2, v3, v0});
			for (GLuint i = startIndex; i < startIndex + 6; i++)
		...
		// Back face (z=z0) -- note winding order is reversed to have
		// the normal point outward.
      ...
      public:
	Prism(glm::vec3 color, glm::vec3 size) : Mesh(color), _dimensions(size)
	{
		_generatePrism();
	}
};
