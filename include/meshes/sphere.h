...
class Sphere : public Mesh
{
		...
		for (int face = 0; face < 6; face++) {
			for (int i = 0; i <= _divisions; i++) {
				for (int j = 0; j <= _divisions; j++) {
					...
					// Calculate vertex position based on
					// face and subdivision
					glm::vec3 vertex;
					switch (face) {
					case 0: // +X face
						vertex = glm::vec3(
					...
					// Add the normalized, scaled vertex to
					// the list
					_vertices.push_back(
					    glm::normalize(vertex) * _radius);
	...
	void _thinVertices()
	{
		std::unordered_set<glm::vec3> uniqueVerts(_vertices.begin(),
							  _vertices.end());

		for (auto &idx : _indices) {
			idx = std::distance(uniqueVerts.begin(),
					    uniqueVerts.find(_vertices[idx]));
		}
