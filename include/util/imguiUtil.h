...
class ImguiUtil
{
	...
	std::map<const char *, float> _fragMap = {{"{{GAMMA}}", 1.8},
						  {"{{SPEC_STRENGTH}}", 0.5},
						  {"{{SHINE_FACTOR}}", 64},
	...
	{
		ImGui_ImplSDL2_InitForOpenGL(window->getWindow(),
					     window->getContext());
	...
	void fragBox(glm::ivec2 pos, glm::ivec2 size, DISPLAY::Shader *shader)
	{
		...
		ImGui::SliderFloat("##GAMMA", &_fragMap.at("{{GAMMA}}"), 0.0f,
				   3.2f);
		ImGui::Spacing();

		ImGui::Text("SPEC STRENGTH:");
		ImGui::SetNextItemWidth(fullWidth);
		ImGui::SliderFloat("##SPEC_STRENGTH",
				   &_fragMap.at("{{SPEC_STRENGTH}}"), 0.0f,
				   1.0f);
		ImGui::Spacing();

		ImGui::Text("SHINE FACTOR:");
		ImGui::SetNextItemWidth(fullWidth);
		if (ImGui::SliderInt("##SHINE", &shineIndex, 0, 9,
				     std::to_string(1 << shineIndex).c_str())) {
			_fragMap.at("{{SHINE_FACTOR}}") =
			    (float)(1 << shineIndex);
		}
		ImGui::Spacing();
		...
		ImGui::SetNextItemWidth(fullWidth);
		ImGui::SliderFloat("##AMBIENT_STRENGTH",
				   &_fragMap.at("{{AMBIENT_STRENGTH}}"), 0.0f,
				   1.0f);
		ImGui::Spacing();

		ImGui::Text("AMBIENT BLEND:");
		ImGui::SetNextItemWidth(fullWidth);
		ImGui::SliderFloat("##AMBIENT_BLEND",
				   &_fragMap.at("{{AMBIENT_BLEND}}"), 0.0f,
				   1.0f);
		ImGui::Spacing();
	...
	std::string replaceFragVars(std::string src)
	{
		for (const auto &[var, val] : _fragMap)
			src.replace(src.find(var), strlen(var),
				    std::to_string(val));
		return src;
	}
