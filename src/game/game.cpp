...
namespace GAME {
    ...
    void Game::_init() {
        ...
        _window.initDisplay("Poly", 640 + _utilWidth, 480, _utilWidth, DISPLAY::WF::TYPICAL);
        
        if (_imgUtil) _imgUtil->init(&_window);
        _timer.init();
        ...
        _walls.insert(_walls.end(), {
                new ENT::Wall({1.0f, 0.3f, 0.3f}, {1.0f, 8.0f, 8.0f}, {2.0f, -4.0f, 0.0f}),
    ...
    void Game::_handleEvents() {
        ...
        while (SDL_PollEvent(&event)) {
            if (_imgUtil) ImGui_ImplSDL2_ProcessEvent(&event);
    ...
    void Game::_update() {
        ...
        for (const auto &wall : _walls) wall->update();
    ...
    void Game::_draw() {
        if (_imgUtil) {
            _imgUtil->begin();
            _imgUtil->fragBox({0, 0}, {_utilWidth, _window.getHeight()}, _shaders.at("obj"));
            _imgUtil->end();
        }

        _light.draw(_shaders.at("light"));
        _player.draw(_shaders.at("obj"), &_light, &_window, _camera.getPos());
        for (const auto &wall : _walls) wall->draw(_shaders.at("obj"), &_light, &_window, _camera.getPos());
