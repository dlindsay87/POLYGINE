...
void Window::_setContextVersion()
{
	...
	if (_context == NULL) {
		cerr << "OpenGL context creation failed: " << SDL_GetError()
		     << endl;
		exit(1);
	}
...
void Window::initDisplay(const char *name, int w, int h, int offset,
			 unsigned int f)
{
	_name = name;
	_width = w, _height = h;
...
void Window::swap() { SDL_GL_SwapWindow(_window); }
