...
namespace DISPLAY {
		...
		void Camera::_initUBO() {
	        ...
	        glBindBufferRange(GL_UNIFORM_BUFFER, 0, _UBO, 0, 2 * sizeof(glm::mat4));// + sizeof(glm::vec3));
	    }
		...
		void Camera::_zoom(const float& aspectRatio) {
			...
			_projMat = glm::perspective(glm::radians(_zoomFactor), aspectRatio, 0.1f, 100.0f);
		}
